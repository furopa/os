...
void showGanttchart();
void showPT();
int getprocessbyPrioP();
int getprocessbyPrioP1();
int sch_cnt,ct,np,i;
void main()
{
	accept();
...
int getprocessbyPrioP1()
{
	...
	for(i=0;i<np;i++)
	{
		if(p[i].at<=ct && p[i].t_bt>0 && p[i].Prio<minat && (ct-p[i].n_bt)>2)
		{
			minp=i;
			minat=p[i].at;
		}
...
void showPT()
{
	...
	printf("\n");
	printf("\nPID\tFBT\tNBT\tAT\tPrio\tFT\tTAT\tWT\n");
	for(i=0;i<np;i++)
	{
		printf("\n%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",i,p[i].f_bt,p[i].n_bt,p[i].at,p[i].Prio,p[i].ft,p[i].tat,p[i].wt);
		avgtat+=p[i].tat;
		avgwt+=p[i].wt;
	}
	avgtat=avgtat/np;
