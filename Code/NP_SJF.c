...
void showGanttchart();
void showPT();
int getprocessbysjfnp();
int getprocessbysjfnp1();
int sch_cnt,ct,np,i;
void main()
{
	accept();
...
int getprocessbysjfnp()
{
	...
	for(i=0;i<np;i++)
	{
		if(p[i].at<=ct && p[i].t_bt>0 && p[i].t_bt<minat)
		{
			minp=i;
			minat=p[i].at;
		}
...
int getprocessbysjfnp1()
{
	...
	for(i=0;i<np;i++)
	{
		if(p[i].at<=ct && p[i].t_bt>0 && p[i].t_bt<minat && (ct-p[i].n_bt)>2)
		{
			minp=i;
			minat=p[i].at;
		}
